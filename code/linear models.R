...
### Richness vs other variables ----
str(merged3)
time<- subset(merged3, select=c(sunrise, time, richness, sunrise_time, site, habitat, day, boats, weather, wind, full_simpson, low_simpson))
View(time)
time$habitat<- as.factor(time$habitat)
time$wind<- as.numeric(time$wind)
# boat
boat<-  time %>%
  group_by(site, wind, habitat, weather,boats) %>%
  summarize(mean = mean(low_simpson)) %>%
  ungroup()
View(wind)
boxplot(boat$mean~boat$boats)
summary(lm(mean~wind, data = wind))

#wind
wind<-  time %>%
  group_by(site, wind, habitat, weather, full_simpson, low_simpson) %>%
  summarize(mean = mean(low_simpson)) %>%
  ungroup()
View(wind)
boxplot(wind$mean~wind$weather)
summary(lm(mean~wind, data = wind))
