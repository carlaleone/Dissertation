...
```
## PCA for the low frequency presence/absence data
```{r}
low_pres<- read_excel("data/meta_richness.xlsx", 
                            sheet = "low_presence")
View(low_pres)

low_pres<- subset(low_pres, select = -c(max_richness, simpsons, habitat) ) #remove unnecessary columnsc
low_pres<- low_pres %>%
  column_to_rownames(var = "site") # make site the name of the rows
# try the PCA
PCA<- rda(low_pres, scale=F)
plot(PCA)
summary(PCA)
biplot(PCA)
```

## PCA for the low frequency relative abundance data
```{r}
low_abundance<- read_excel("data/meta_richness.xlsx", 
...
# try the PCA
PCA2<- rda(low_abundance, scale=F)
plot(PCA)
summary(PCA2)
#very low eigenvalues
#very low loadings for the sites for each PC
biplot(PCA2)
screeplot(PCA2)
# Not worth more than PC2
```
## NMDS Total Richness, low frequency band
```{r}
...
low_richness<- subset(low_richness, select = -c(max_richness,simpsons, habitat) ) #remove unnecessary columnsc
...
habitats<- subset(habitats, select = c(site, habitat))

### Distance matrix calculation
low_richness_dist<- vegdist(presence_nmds, method="jaccard", binary= TRUE)
low_richness_dist

### NMDS
low_richness_nmds<- metaMDS(low_richness, #the community data
                    distance = "jaccard", # Using bray-curtis distance
                    try = 100)

plot(low_richness_nmds, type="t")
low_richness_nmds
```
...
### SIMPER
```{r}
basic_simper_<- simper(max_richness, #our community data set
